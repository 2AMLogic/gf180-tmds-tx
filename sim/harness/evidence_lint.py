...
- a record whose per-corner logs carry a bit-rate token (a transient/eye
  measurement run against one of spec/tmds-tx.md §1's two operating points)
  has non-empty **Operating point** and **Transient settings** fields, and
  the rate(s) declared in **Operating point** match every log's rate token
  exactly -- this is what makes a record covering only the 480p fallback
  unable to pass while its **Operating point** field claims the 720p60
  target row (see ``_check_operating_point``);
...
What is deliberately *not* checked: the prose inside a field beyond the
mechanical checks listed above (a record is a human/agent-written argument,
not a form), and files under ``corners/<id>/`` that are not ``*.log`` (the
layout in ``sim/README.md`` names the logs; it does not forbid a future
sidecar artefact, and a checker that guessed here would block legitimate
evidence).

Ported from `2AMLogic/gf180-bandgap` (sim/harness/evidence_lint.py). The
corner-id grammar gains an optional trailing ``<rate>`` field (this repo's
transient/bit-rate axis, see ``sim/harness/corners.py``'s module docstring)
and the two-field cross-check in ``_check_operating_point``; nothing else
changes.
"""
...
#: **Operating point** / **Transient settings** are a conditionally-required
#: tenth/eleventh field (only for a rate-bearing record) and are therefore
#: checked separately by ``_check_operating_point``, not listed here.
...
#
# `<process>_<temp>c_<supply>[_<rate>]`, split on the *last two or three*
# underscores so the process field may itself carry a device-family prefix
# (`bjt_ff`, `res_typical`). The process vocabulary is deliberately open:
# gf180mcu names a model section per device family, so a device-level
# testbench selects `typical` / `bjt_ff` / `res_ss` rather than one of the
# harness's five MOS corner names.
...
NO_SUPPLY = "nosupply"
#: The optional trailing bit-rate field this repo adds to the grammar --
#: `742p5mbps`, `270mbps` (`p` stands in for the decimal point, same
#: convention as `<supply>`). See `sim/harness/corners.py`'s
#: `_format_rate_token` and the module docstring above.
_RATE_RE = re.compile(r"^\d+(?:p\d+)?mbps$")
#: `742.5 Mbps` / `270Mbps` inside a free-text **Operating point** value --
#: used to cross-check the declared rate(s) against the per-corner logs'
#: rate tokens. See `_check_operating_point`.
_OPERATING_POINT_RATE_RE = re.compile(r"(\d+(?:\.\d+)?)\s*Mbps", re.IGNORECASE)
...
def _split_rate_token(corner_id: str) -> tuple[str, str | None]:
    """Strip a trailing ``<rate>mbps`` token, if present and well-formed.

    Returns ``(remainder, rate_token)``; ``rate_token`` is ``None`` when
    ``corner_id`` carries no trailing rate field at all (the un-adapted
    grammar's shape).
    """
    parts = corner_id.split("_")
    if len(parts) >= 4 and _RATE_RE.match(parts[-1]):
        return "_".join(parts[:-1]), parts[-1]
    return corner_id, None


def decode_rate_token(token: str) -> float:
    """``742p5mbps`` -> ``742.5``, ``270mbps`` -> ``270.0``."""
...
def parse_corner_id(corner_id: str) -> str | None:
    ...
    Splitting on the *last two* (or, with a rate field, three) underscores is
    what lets the process field carry a device-family prefix; neither temp,
    supply, nor rate may contain one.
    """
    remainder, rate_token = _split_rate_token(corner_id)
    parts = remainder.split("_")
    if len(parts) < 3:
        return (
            f"{corner_id!r} does not parse as <process>_<temp>c_<supply>[_<rate>] "
    ...
    if rate_token is not None and not _RATE_RE.match(rate_token):
        return (  # pragma: no cover - _split_rate_token already validated this
            f"{corner_id!r}: trailing rate field {rate_token!r} is not "
            "<digits>[p<digits>]mbps (742p5mbps, 270mbps)"
        )
    return None
...
def _check_operating_point(
    path: str, fields: dict, rate_tokens: set[str]
) -> list:
    """**Operating point** / **Transient settings** for a rate-bearing record.

    ``rate_tokens`` is the set of trailing ``<rate>`` tokens actually found on
    this record's ``corners/<record-id>/*.log`` names (empty for a DC/
    op-point record, which skips this check entirely -- the two fields do not
    exist in that case, same as the un-adapted source harness's records).
    """
    if not rate_tokens:
        return []
    problems: list = []
    op_field = fields.get("Operating point")
    if op_field is None or not op_field.value:
        problems.append(
            Problem(
                path,
                "corner logs carry a rate (Mbps/lane) token but the record has no "
                "non-empty **Operating point** field declaring which spec row "
                "(target 742.5 Mbps/720p60 or fallback 270 Mbps/480p, "
                "spec/tmds-tx.md §1) this record substantiates",
            )
        )
    else:
        declared = {
            round(float(m), 6) for m in _OPERATING_POINT_RATE_RE.findall(op_field.value)
        }
        if not declared:
            problems.append(
                Problem(
                    path,
                    f"**Operating point** value {op_field.value!r} names no numeric "
                    "Mbps/lane rate",
                )
            )
        else:
            observed = {round(decode_rate_token(t), 6) for t in rate_tokens}
            if observed != declared:
                problems.append(
                    Problem(
                        path,
                        f"**Operating point** declares {sorted(declared)} Mbps/lane but "
                        f"corner logs carry rate(s) {sorted(observed)} Mbps/lane -- a "
                        "record covering only the fallback rate cannot claim the target "
                        "row (or vice versa); every rate actually run must be declared "
                        "and every declared rate must actually have been run",
                    )
                )
    ts_field = fields.get("Transient settings")
    if ts_field is None or not ts_field.value:
        problems.append(
            Problem(
                path,
                "corner logs carry a rate (Mbps/lane) token (a transient/eye "
                "measurement run) but the record has no non-empty **Transient "
                "settings** field (max timestep, .tran step/stop, "
                "reltol/abstol/vntol -- see sim/README.md)",
            )
        )
    return problems
...
def check_record(
    ...
    rate_tokens: set[str] = set()
    for name in sorted(logs):
        corner_id = name[: -len(LOG_SUFFIX)]
        reason = parse_corner_id(corner_id)
        if reason:
            problems.append(Problem(corner_dir + name, reason))
            continue
        _, rate_token = _split_rate_token(corner_id)
        if rate_token is not None:
            rate_tokens.add(rate_token)
    problems += _check_operating_point(path, fields, rate_tokens)
