...
Ported from `2AMLogic/gf180-bandgap` (sim/harness/report.py). ``render_record``
gains two conditional fields -- **Operating point** and **Transient
settings** -- emitted only when the run's grid carries a bit-rate axis (see
``sim/harness/corners.py``/``sim/harness/testbench.py``'s module docstrings);
a DC/op-point record renders identically to the un-adapted source harness.
"""
...
def matrix_conformance(tb: Testbench, points: list[PvtPoint]) -> dict:
    """Is this run the full PVT (x rate) matrix CLAUDE.md/sim/README.md
    mandates, or a subset?
    ...
    record states why a subset was used, plus -- for a testbench that
    declares a ``rates_mbps`` axis -- every declared rate. This returns what
    is missing so the CLI can insist on a written justification instead of
    quietly recording a thinner run.
    """
    temps = {round(p.temp_c, 6) for p in points}
    supplies = {round(p.vdd, 6) for p in points}
    process = {p.corner.name for p in points}
    rates = {round(p.rate_mbps, 6) for p in points if p.rate_mbps is not None}
    ...
    required_rates = {round(r, 6) for r in tb.rates_mbps}
    ...
    if not required_rates <= rates:
        missing.append(
            "rate: missing "
            + ", ".join(f"{r:g} Mbps/lane" for r in sorted(required_rates - rates))
        )
...
def build_record(
    ...
    rates = sorted({p.rate_mbps for p in points if p.rate_mbps is not None})
...
def _corner_matrix_lines(record: dict) -> list[str]:
    ...
    if grid["rates_mbps"]:
        lines.append("  - Rate: " + ", ".join(f"{r:g} Mbps/lane" for r in grid["rates_mbps"]))
    axes = "process × temperature × supply" + (" × rate" if grid["rates_mbps"] else "")
    lines.append(
        f"  - {grid['points']} point full-factorial grid ({axes}), {grid['points_ok']} completed"
    )
    if record["matrix"]["full"]:
        lines.append(
            "  - Full PVT matrix per CLAUDE.md (−40/27/125 °C, ±10 % supply, "
            "process corners" + (", every declared rate" if grid["rates_mbps"] else "") + ")."
        )
    else:
...
def _operating_point_label(rate_mbps: float) -> str:
    """``742.5`` -> ``target (720p60)``, ``270.0`` -> ``fallback (480p)``,
    anything else -> ``custom`` -- see spec/tmds-tx.md §1.
    """
    if abs(rate_mbps - RATE_TARGET_MBPS) < 1e-6:
        return "target (720p60)"
    if abs(rate_mbps - RATE_FALLBACK_MBPS) < 1e-6:
        return "fallback (480p)"
    return "custom"


def _operating_point_lines(record: dict) -> list[str]:
    """**Operating point** / **Transient settings** -- only rendered when the
    run's grid carries a bit-rate axis (see corners.py/testbench.py module
    docstrings and sim/README.md's corner-id grammar). Both fields are
    required, and cross-checked against the per-corner logs' rate tokens, by
    ``sim/harness/evidence_lint.py`` whenever they apply.
    """
    rates = record["grid"]["rates_mbps"]
    if not rates:
        return []
    labels = ", ".join(f"{r:g} Mbps/lane ({_operating_point_label(r)})" for r in rates)
    lines = [f"- **Operating point**: {labels}"]
    t = record["testbench"].get("transient", {})
    settings = [
        f"tstep={_fmt(t.get('tstep_s'))}s",
        f"tstop={_fmt(t.get('tstop_s'))}s",
        f"tmax={_fmt(t.get('tmax_s'))}s",
        f"reltol={_fmt(t.get('reltol'))}",
        f"abstol={_fmt(t.get('abstol'))}",
        f"vntol={_fmt(t.get('vntol'))}",
    ]
    if t.get("pattern"):
        settings.append(f"pattern={t['pattern']}")
    lines.append("- **Transient settings**: " + " ".join(settings))
    return lines
...
def render_record(record: dict, experiment: str) -> str:
    ...
    provenance, Corner matrix run, [Operating point, Transient settings --
    only for a rate-bearing run], Statistical convention, Result, Links,
    ...
    lines += _operating_point_lines(record)
    lines.append(
...
#
# The ``sim/device-*/run_*.py`` experiments (none exist in this repo yet) do
...
def device_write_record(records_dir: Path, record: str, body: str) -> Path:
    ...
    a ``device-*`` experiment composes its own pre-rendered markdown ``body``
    string and just needs an append-only-guarded string writer.
    """
