...
Ported from `2AMLogic/gf180-bandgap` (sim/harness/cli.py). ``run`` gains the
bit-rate axis: when a testbench declares ``rates_mbps`` (see
``sim/harness/corners.py``/``sim/harness/testbench.py``'s module docstrings)
every point in the grid is cross-producted with that axis too, and the
progress/summary output reports it alongside process/temperature/supply. A
testbench with no ``rates_mbps`` behaves identically to the un-adapted
source harness.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="run_corners.py",
        description="Run a testbench across the gf180mcu PVT (and, for a "
        "transient bench, bit-rate) corner grid.",
    ...
    parser.add_argument(
        ...
        type=float,
        metavar="MBPS",
        help="bit rates in Mbps/lane (overrides the manifest's 'rates_mbps'; "
        "only meaningful for a testbench that declares a rate axis)",
    )
    ...
    parser.add_argument(
        "--claim",
        default="",
        help="spec line this run substantiates, e.g. 'spec/tmds-tx.md#dr-0002' "
    ...
    parser.add_argument(
        "--version", action="version", version=f"gf180-tmds-tx harness {HARNESS_VERSION}"
    )
    return parser
...
def cmd_list() -> int:
    ...
    for directory in tb_mod.discover(SIM_DIR):
        try:
            ...
            rate_note = f"  [rates: {', '.join(f'{r:g}' for r in tb.rates_mbps)} Mbps/lane]" if tb.rates_mbps else ""
            print(f"  {directory.name:<20} {tb.description or tb.name}{rate_note}")
...
def cmd_check_env() -> int:
    ...
    try:
        pdk = find_pdk()
        print(f"PDK     : OK   {pdk.variant} ({pdk.path}, open_pdks {pdk.version}, via {pdk.source})")
...
def run(args: argparse.Namespace) -> int:
    ...
    rates = args.rates if args.rates is not None else (list(tb.rates_mbps) or None)
    points = corners_mod.build_grid(corner_list, temperatures, supplies, rates)

    # sim/README.md: an evidence record must cover the full mandated PVT (and,
    # for a transient bench, rate) matrix unless it states why a subset was
    # used. Refuse to record a thin run without that justification rather
    # than quietly banking weak evidence.
    ...
    def progress(result):
        ...
        print(f"[{completed:>3}/{len(points)}] {flag} {result.point.corner_id:<32} {detail}")
