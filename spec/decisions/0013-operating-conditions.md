...
| 11 | ESD | HBM >= 2 kV, CDM >= 500 V | DR-0005/0011/0015 (Accepted, unrelaxed) | **Accepted pre-silicon limitation, not open work (DR-0015)** — no PDK source characterizes ESD failure current density, breakdown voltage, or snapback/trigger behavior for any device family (`design/esd-capacitance-budget.md` §2a), so real HBM/CDM qualification cannot happen before silicon exists. The clamp-device *class* is signed off (DR-0011) and the HBM sizing estimate (§2b) is measured against the capacitance budget; DR-0015 records this design-margin estimate as the accepted evidence and closes the row's tracking ambiguity without relaxing the requirement. Re-opens on real silicon/ESD-tester data or a PDK-sourced ESD electrical dataset, per DR-0015's Consequences |
